...
import { LockKey, MagicWand, PaperPlaneTilt } from "@phosphor-icons/react/dist/ssr";
import { PrintButton } from "@/components/CountedTextarea";
import { Callout, Card, FictionBanner, Pill, ScreenHeader, StickyAction } from "@/components/ui";
...
export default async function ApplyPage({ params }: { params: Promise<{ trialId: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {trial.isFictional ? <FictionBanner>{form.notice}</FictionBanner> : <Callout tone="caution">{form.notice} Nothing is sent to a real site from this prototype.</Callout>}

      <div className="flex items-start gap-3 rounded-[16px] bg-mint-soft px-4 py-3.5">
        <MagicWand size={22} weight="fill" className="mt-0.5 shrink-0 text-mint" />
        <div>
          <p className="text-[14px] font-bold text-ink">We filled in {filled} of {main.length} answers for you</p>
          <p className="text-[12.5px] leading-relaxed text-ink-soft">
            From your passport, so you don&rsquo;t retype them. {unknown ? `${unknown} you marked as unknown are left blank. ` : ""}
            Check each one. Anything you leave empty isn&rsquo;t sent.
          </p>
        </div>
      </div>
