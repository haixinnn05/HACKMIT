...
import {
  ...
  Heartbeat, Lightbulb, MagicWand, PaperPlaneTilt, ShieldCheck, Sparkle, Timer,
...
export default async function TrialPage({
  params, searchParams,
}: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; ask?: string; plain?: string }> }) {
  const { id } = await params;
  const { tab: rawTab, ask, plain } = await searchParams;
        ...
        {ask?.trim() ? (
          <Suspense key={ask} fallback={
            <Card className="mt-2.5 space-y-2 p-4" >
              ...
              <p className="text-[13px] text-ink-soft">Reading this study&rsquo;s record for you&hellip;</p>
...
function BriefView({ brief, writing = false }: { brief: TrialBrief; writing?: boolean }) {
  return (
    <div>
      <SectionHeading trailing={writing ? "Writing a plain-language version, up to a minute" : undefined}>Study summary</SectionHeading>
      ...
      <details className="group mt-1">
        ...
        <div className="space-y-3 pb-1">
          ...
          <p className="text-[11.5px] leading-relaxed text-ink-faint">
            {brief.mode === "model"
              ? `Written by ${AI_METADATA.model} from the sources above, and every quote was checked against them.${brief.droppedClaims > 0 ? ` ${brief.droppedClaims} statement${brief.droppedClaims === 1 ? " was" : "s were"} removed because the quoted text was not found in the source.` : ""}`
  ...
  return (
    <Card className="animate-rise mt-2.5 p-4" >
      ...
      <p className="mt-2 text-[11px] text-ink-faint">
        {reply.mode === "model" ? "Answered with a language model, and the quote was checked against the record." : "Found by keyword search. No language model was involved."}
      </p>
    </Card>
  );
}
