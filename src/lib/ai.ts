...
function sourcesFor(trial: Trial): SourceDoc[] {
  ...
  if (trial.briefSummary) {
    docs.push({
      id: `${trial.id}#summary`,
      label: `Record ${trial.id}, brief summary`,
      version,
  ...
  if (trial.eligibilityText) {
    docs.push({
      ...
      label: `Record ${trial.id}, eligibility criteria`,
      version,
  ...
  if (trial.knownLogistics?.compensationText) {
    docs.push({
      ...
      label: `Study site ${trial.id}, logistics`,
      version,
  ...
  if (trial.detailedDescription) {
    docs.push({
      ...
      label: `Record ${trial.id}, detailed description`,
      version,
...
Give 3 to 5 claims and 3 to 5 questions. Prefer questions the sources leave unanswered.`,
  ...
  return {
    ...
    notice: `Written from the record for ${trial.id}, last updated ${trial.lastUpdatePostDate ?? "unknown"}.`,
  };
}
...
export function composeOfflineBrief(
  ...
  return {
    ...
    whatParticipationInvolves: visitsKnown
      ? "A visit schedule is available for this study. What to Expect turns it into hours."
    ...
    droppedClaims: 0,
    latencyMs,
    notice: `Assembled from the record for ${trial.id}.`,
  };
}
